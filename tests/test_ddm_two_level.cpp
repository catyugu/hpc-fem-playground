/**
 * @file test_ddm_two_level.cpp
 * @brief Test for two-level overlapping Schwarz with AMG coarse solver
 * 
 * Phase: 2, Step: 2.2
 * 
 * This test verifies correctness of the two-level domain decomposition method
 * on a 3D Poisson problem with manufactured solution. The two-level method adds
 * a coarse-grid correction to the one-level method, providing scalability.
 * 
...
int main(int argc, char *argv[])
{
    ...
    if (myid == 0)
    {
        std::cout << "Testing TwoLevelSchwarz with " << numProcs << " MPI processes" << std::endl;
    }
    ...
    // Create two-level Schwarz preconditioner with coarse-grid correction
    TwoLevelSchwarz prec(A, &fespace);

    // Solve using CG with two-level Schwarz preconditioner
    ...
    if (myid == 0)
    {
        if (test_passed)
        {
            std::cout << "Test passed: TwoLevelSchwarz correctness test" << std::endl;
        }
        else
        {
