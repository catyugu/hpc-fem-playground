...
if(ENABLE_MFEM_MPI)
    if(NOT VENDOR_THIRD_PARTY)
        ...
        # HYPRE
        set(HYPRE_SRC ${THIRD_PARTY_DIR}/hypre)
        if(NOT EXISTS ${HYPRE_SRC})
            message(STATUS "Cloning HYPRE into ${HYPRE_SRC}")
            execute_process(COMMAND git clone --depth 1 https://github.com/hypre-space/hypre.git ${HYPRE_SRC}
                            --branch v2.33.0    RESULT_VARIABLE _gitret_hypre
                            OUTPUT_QUIET ERROR_QUIET)
            if(NOT _gitret_hypre EQUAL 0)
                message(WARNING "Failed to clone HYPRE into ${HYPRE_SRC}")
            endif()
        endif()
        if(NOT EXISTS ${HYPRE_SRC})
            message(WARNING "HYPRE not available; disabling MFEM MPI support")
            set(_mpi_deps_ok FALSE)
        else()
            set(HYPRE_INCLUDE_DIR ${HYPRE_SRC}/src)
        endif()
